...
<div class="hs-dropdown relative inline-flex">
  ...
  >
    @foreach ($languages as $lang => $label)
      <a
        class="flex items-center gap-x-3.5 rounded-lg px-3 py-2 text-sm text-neutral-800 hover:bg-neutral-100 focus:bg-neutral-100 focus:outline-hidden dark:text-neutral-400 dark:hover:bg-neutral-700 dark:hover:text-neutral-300 dark:focus:bg-neutral-700 {{ $locale === $lang ? 'font-bold text-orange-400' : '' }}"
        href="{{ $lang === 'en' ? '/' : '/fr' }}"
        data-lang="{{ $lang }}"
      >
        {{ $label }}
